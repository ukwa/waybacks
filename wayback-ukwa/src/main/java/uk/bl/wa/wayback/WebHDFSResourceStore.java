...
public class WebHDFSResourceStore extends SimpleResourceStore {
     ...
     * 
     * @see
     * org.archive.wayback.resourcestore.SimpleResourceStore#retrieveResource(
	...
	public Resource retrieveResource(CaptureSearchResult result)
        ...
        if (result.getCompressedLength() > 0) {
            fileName = fileName + "?user.name=" + this.hdfsUser
                    + "&op=OPEN&offset=" + result.getOffset() + "&length="
                    + result.getCompressedLength() + "#.warc.gz";
        } else {
            fileName = fileName + "?user.name=" + this.hdfsUser
                    + "&op=OPEN&offset="
                + result.getOffset() + "#.warc.gz";
        }
